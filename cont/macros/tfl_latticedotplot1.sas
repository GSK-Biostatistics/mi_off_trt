/*******************************************************************************
|
| Program Name   : tfl_latticedotplot1.sas
   ...
   begingraph /  pad=(top=8pct right=8pct)   
      ...
      layout datalattice rowvar=withtype columnvar=discrate /             
        ...
        rowaxisopts=(display=all griddisplay=on label=YLABEL
          linearopts=(viewmin=YMIN viewmax=YMAX minorgrid=on 
                      tickvaluesequence=(start=YMIN end=YMAX increment=YBY)))
        ...
        layout prototype;
          ...
          %if %length(&yref.) ne 0 %then %do; referenceline y = &yref. / lineattrs=(color=black pattern=3); %end;    
          ...
          %if %length(&ytvar.) ne 0 %then %do;
            scatterplot x = XVAR y = YTVAR / group = ZVAR datalabel = YTLABEL datalabelposition=bottom
            markerattrs=(symbol=triangle size=&dsize. weight=bold)
...
proc sgrender data=&indata. sganno=annotate template=latticeplot1;
